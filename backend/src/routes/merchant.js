...
function ensureApproved(merchantId) {
...
router.post('/kyc/submit', verifyToken, requireRole('merchant'), (req, res, next) => {
  try {
    const schema = z.object({ business_doc_url: z.string(), identity_doc_url: z.string() });
    const data = schema.parse(req.body);
    const m = db().merchants.find(x => x.id === req.user.sub);
    if (!m) return res.status(404).json({ error: 'Not found' });
    Object.assign(m, { ...data, kyc_status: 'pending' });
    saveDB(db());
    res.json({ ok: true, kyc_status: m.kyc_status });
  } catch (e) { next(e); }
});

router.get('/dashboard', verifyToken, requireRole('merchant'), (req, res) => {
  const store = db();
  const id = req.user.sub;
  res.json({
    vehicles: store.vehicles.filter(v => v.merchant_id === id).length,
    bookings: store.bookings.filter(b => b.merchant_id === id).length,
    earnings: store.bookings.filter(b => b.merchant_id === id).reduce((s, b) => s + b.merchant_payout, 0),
  });
});

router.post('/vehicles', verifyToken, requireRole('merchant'), (req, res, next) => {
  try {
    ensureApproved(req.user.sub);
...
router.get('/vehicles', verifyToken, requireRole('merchant'), (req, res) => {
  const store = db();
  res.json(
    store.vehicles
...
router.put('/vehicles/:id', verifyToken, requireRole('merchant'), (req, res, next) => {
  try {
    ensureApproved(req.user.sub);
    const data = vehicleSchema.partial().parse(req.body);
    const v = db().vehicles.find(x => x.id === req.params.id && x.merchant_id === req.user.sub);
...
router.delete('/vehicles/:id', verifyToken, requireRole('merchant'), (req, res) => {
  const store = db();
  const i = store.vehicles.findIndex(v => v.id === req.params.id && v.merchant_id === req.user.sub);
  if (i < 0) return res.status(404).json({ error: 'Not found' });
  store.vehicles.splice(i, 1);
  saveDB(store);
  res.json({ ok: true });
});

router.get('/bookings', verifyToken, requireRole('merchant'), (req, res) => {
  const store = db();
...
router.get('/earnings', verifyToken, requireRole('merchant'), (req, res) => {
  const bs = db().bookings.filter(b => b.merchant_id === req.user.sub);
  const gross = bs.reduce((s, b) => s + b.total_amount, 0);
  const commission = bs.reduce((s, b) => s + b.commission_amount, 0);
  const payout = bs.reduce((s, b) => s + b.merchant_payout, 0);
  const paid = bs.filter(b => b.payment_status === 'fully_paid' || b.payment_status === 'advance_paid');
  const paidGross = paid.reduce((s, b) => s + b.total_amount, 0);
  const paidCommission = paid.reduce((s, b) => s + b.commission_amount, 0);
  const paidPayout = paid.reduce((s, b) => s + b.merchant_payout, 0);
  res.json({
